...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
            ...
            <p class="text-lg font-bold text-gray-900 tabular-nums"><?php echo e($summary['total']); ?></p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
            <p class="text-xs text-gray-500">Actifs</p>
            <p class="text-lg font-bold text-emerald-600 tabular-nums"><?php echo e($summary['active']); ?></p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
            <p class="text-xs text-gray-500">Vendables</p>
            <p class="text-lg font-bold text-blue-600 tabular-nums"><?php echo e($summary['sellable']); ?></p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
            <p class="text-xs text-gray-500">Achetables</p>
            <p class="text-lg font-bold text-indigo-600 tabular-nums"><?php echo e($summary['purchasable']); ?></p>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5"><?php echo e($products->total()); ?> article(s) trouvé(s)</p>
        </div>
        <div class="flex items-center gap-2 self-start flex-wrap">
            <a href="<?php echo e(route('exports.products', request()->query())); ?>"
            ...
            <a href="<?php echo e(route('exports.products-pdf', request()->query())); ?>"
            ...
            <a href="<?php echo e(route('import.index', ['type' => 'products'])); ?>"
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-200 p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3">
            ...
            <select name="family_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                ...
                <?php $__currentLoopData = $families; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $family): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <option value="<?php echo e($child->id); ?>" <?php echo e(request('family_id') == $child->id ? 'selected' : ''); ?>>&nbsp;&nbsp;└ <?php echo e($child->name); ?></option>
            ...
            <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                ...
                <option value="compose" <?php echo e(request('type') === 'compose' ? 'selected' : ''); ?>>Composé</option>
            </select>
        ...
        <div class="flex items-center gap-2 mt-3">
            ...
            <?php if(request()->hasAny(['search', 'family_id', 'brand_id', 'type', 'is_active'])): ?>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <img src="<?php echo e(url(Storage::url($product->image))); ?>" class="w-9 h-9 rounded-lg object-cover flex-shrink-0 border border-gray-100">
                                <?php else: ?>
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 font-mono"><?php echo e($product->reference ?: '—'); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 hidden md:table-cell">
                            <?php if($product->family): ?>
                                <span class="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full"><?php echo e($product->family->name); ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-4 py-3 hidden lg:table-cell">
                            ...
                            ?>
                            <span class="text-xs font-medium px-2 py-0.5 rounded-full <?php echo e($t['class']); ?>"><?php echo e($t['label']); ?></span>
                        </td>
                        <td class="px-4 py-3 text-right text-gray-500 tabular-nums text-sm whitespace-nowrap hidden sm:table-cell">
                            <?php echo e($product->purchase_price > 0 ? number_format($product->purchase_price, 0, ',', ' ').' FCFA' : '—'); ?>

                        </td>
                        <td class="px-4 py-3 text-right font-semibold text-gray-900 tabular-nums whitespace-nowrap">
                            <?php echo e($product->sale_price > 0 ? number_format($product->sale_price, 0, ',', ' ').' FCFA' : '—'); ?>

                        </td>
                        <td class="px-4 py-3 text-center text-gray-500 text-xs hidden xl:table-cell">
                            <?php echo e($product->taxRate?->short_name ?? '—'); ?>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form action="<?php echo e(route('products.destroy', $product)); ?>" method="POST"
                                      onsubmit="return confirm('Supprimer « <?php echo e(addslashes($product->name)); ?> » ?')">
                        ...
                        <td colspan="8" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center gap-3 text-gray-400">
                                ...
                                <div>
                                    ...
                                    <?php if(request()->hasAny(['search', 'family_id', 'brand_id', 'type', 'is_active'])): ?>
                                        <a href="<?php echo e(route('products.index')); ?>" class="text-sm text-blue-600 hover:text-blue-700 mt-1 inline-block">Effacer les filtres</a>
                                    <?php else: ?>
                                        <a href="<?php echo e(route('products.create')); ?>" class="text-sm text-blue-600 hover:text-blue-700 mt-1 inline-block">Créer le premier article</a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
...
<?php echo $__env->make('layouts.erp', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\iboa\resources\views/products/index.blade.php ENDPATH**/ ?>