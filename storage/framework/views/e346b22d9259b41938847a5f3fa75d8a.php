<?php $__env->startSection('title', 'Facture '.$invoice->number); ?>

<?php $__env->startSection('breadcrumb'); ?>
    <a href="<?php echo e(route('dashboard')); ?>" class="hover:text-gray-700">Accueil</a>
    <span class="mx-1">/</span>
    <a href="<?php echo e(route('ventes.factures.index')); ?>" class="hover:text-gray-700">Factures</a>
    <span class="mx-1">/</span>
    <span class="text-gray-900 font-medium"><?php echo e($invoice->number); ?></span>
...
<div class="space-y-6"
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-2xl font-bold text-gray-900 font-mono"><?php echo e($invoice->number); ?></h1>
                <?php
                    $statusBadges = [
                ...
                ?>
                <span class="badge <?php echo e($statusBadges[$invoice->status] ?? 'badge-gray'); ?>">
                ...
                <?php if($invoice->status === 'payee' || (int) $invoice->remaining_amount === 0): ?>
                ...
                <?php endif; ?>
                <span class="text-gray-500 text-sm"><?php echo e($invoice->client?->name); ?></span>
            </div>

            <div class="flex flex-wrap items-center gap-2">
                
                <a href="<?php echo e(route('ventes.factures.pdf', $invoice)); ?>" class="btn btn-secondary" title="Télécharger le PDF"
                ...
                <a href="<?php echo e(route('ventes.factures.pdf', $invoice)); ?>?preview=1" target="_blank" class="btn btn-secondary" title="Aperçu PDF">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php if($invoice->status === 'brouillon'): ?>
                    ...
                    <form action="<?php echo e(route('ventes.factures.submit', $invoice)); ?>" method="POST"
                ...
                <?php if($invoice->status === 'en_attente_validation'): ?>
                    ...
                    <form action="<?php echo e(route('ventes.factures.validate-internal', $invoice)); ?>" method="POST"
                    ...
                    <form action="<?php echo e(route('ventes.factures.reject-internal', $invoice)); ?>" method="POST"
                    ...
                    <form action="<?php echo e(route('ventes.factures.cancel-internal', $invoice)); ?>" method="POST"
                          x-data="{ open: false, motif: '' }"
                          @submit.prevent="if(motif.trim().length < 5){ alert('Motif obligatoire'); return; } $el.submit()">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="motif" x-model="motif">
                ...
                <?php if($invoice->client?->email && $invoice->status !== 'brouillon'): ?>
                <form action="<?php echo e(route('ventes.factures.send-email', $invoice)); ?>" method="POST"
                      onsubmit="return confirm('Envoyer la facture à <?php echo e(addslashes($invoice->client->email)); ?> ?')">
                    <?php echo csrf_field(); ?>
                ...
                ?>
                <?php if($canEncaisser): ?>
                <a href="<?php echo e(route('tresorerie.encaissements.create', ['client_id' => $invoice->client_id, 'invoice_id' => $invoice->id])); ?>"
                ...
                <?php elseif(!$preValidation): ?>
                
                <button type="button" disabled
                        title="<?php echo e($disabledReason); ?>"
                        aria-disabled="true"
                ...
                <?php if(!in_array($invoice->status, ['brouillon', 'annulee'])): ?>
                <a href="<?php echo e(route('ventes.avoirs.create', ['invoice_id' => $invoice->id])); ?>" class="btn btn-purple">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php if($invoice->status === 'brouillon'): ?>
                <a href="<?php echo e(route('ventes.factures.edit', $invoice)); ?>" class="btn btn-primary">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php if($invoice->type === 'proforma' && in_array($invoice->status, ['emise', 'envoyee'])): ?>
                <form action="<?php echo e(route('ventes.factures.convert-proforma', $invoice)); ?>" method="POST"
                ...
                <?php if(in_array($invoice->status, ['emise', 'envoyee', 'en_retard']) && $invoice->paid_amount == 0): ?>
                ...
                <a href="<?php echo e(route('ventes.factures.index')); ?>" class="btn btn-secondary">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-4 bg-gray-100 border-l-4 border-gray-800 rounded-lg p-3 flex items-start gap-3">
            ...
            <div class="text-sm text-gray-800">
                ...
                <p class="text-xs text-gray-600 mt-0.5">
                    Cette facture ne peut plus recevoir de nouveau paiement (total payé : <?php echo e(number_format($invoice->paid_amount, 0, ',', ' ')); ?> FCFA, reste à payer : 0).
                    En cas de paiement excédentaire reçu, créez un <a href="<?php echo e(route('ventes.avoirs.create', ['invoice_id' => $invoice->id])); ?>" class="underline font-medium">avoir client</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <dd class="mt-0.5 font-semibold text-gray-900"><?php echo e($invoice->client?->name ?? '—'); ?></dd>
                    <?php if($invoice->client?->phone): ?><dd class="text-gray-500 text-xs"><?php echo e($invoice->client->phone); ?></dd><?php endif; ?>
                    <?php if($invoice->client?->email): ?><dd class="text-gray-500 text-xs"><?php echo e($invoice->client->email); ?></dd><?php endif; ?>
                    <?php
                    ...
                    <dd class="text-gray-500 text-xs mt-1 leading-snug">
                        <?php if($addr->address): ?><span><?php echo e($addr->address); ?></span><br><?php endif; ?>
                        <?php if($addr->city || $addr->country): ?>
                            <span><?php echo e(implode(', ', array_filter([$addr->city, $addr->country]))); ?></span>
                        <?php endif; ?>
                    </dd>
                    <?php elseif($invoice->client?->address || $invoice->client?->city): ?>
                    <dd class="text-gray-500 text-xs mt-1 leading-snug">
                        <?php if($invoice->client->address): ?><span><?php echo e($invoice->client->address); ?></span><br><?php endif; ?>
                        <?php if($invoice->client->city || $invoice->client->country): ?>
                            <span><?php echo e(implode(', ', array_filter([$invoice->client->city, $invoice->client->country]))); ?></span>
                        <?php endif; ?>
                    </dd>
                    <?php endif; ?>
                </div>
                <div>
                    ...
                    <dd class="mt-0.5 font-mono font-semibold text-gray-900"><?php echo e($invoice->number); ?></dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wider">Date d'émission</dt>
                    <dd class="mt-0.5 text-gray-700"><?php echo e($invoice->issued_at?->format('d/m/Y') ?? '—'); ?></dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wider">Date d'échéance</dt>
                    <dd class="mt-0.5 <?php echo e($isOverdue ? 'text-red-600 font-semibold' : 'text-gray-700'); ?>">
                ...
                <div>
                    ...
                    <dd class="mt-0.5 text-gray-700"><?php echo e($invoice->payment_terms); ?></dd>
                </div>
                <?php endif; ?>
                ...
                <div>
                    ...
                    <dd class="mt-0.5">
                        <a href="<?php echo e(route('ventes.commandes.show', $invoice->order)); ?>" class="text-blue-600 hover:underline font-mono"><?php echo e($invoice->order->number); ?></a>
                    </dd>
                </div>
                <?php endif; ?>
                ...
                <div class="sm:col-span-2">
                    ...
                    <dd class="mt-0.5 text-gray-700 whitespace-pre-wrap text-xs"><?php echo e($invoice->notes); ?></dd>
                </div>
                <?php endif; ?>
            </dl>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3 h-fit">
            ...
            <div class="flex justify-between text-sm text-gray-600">
                ...
                <span class="font-medium tabular-nums"><?php echo e(number_format($invoice->subtotal_ht, 0, ',', ' ')); ?> FCFA</span>
            </div>
            <div class="flex justify-between text-sm text-gray-600">
                <span>TVA</span>
                <span class="font-medium tabular-nums"><?php echo e(number_format($invoice->total_tax, 0, ',', ' ')); ?> FCFA</span>
            </div>
            <?php if($invoice->global_discount_amount > 0): ?>
            <div class="flex justify-between text-sm text-gray-600">
                <span>Remise globale</span>
                <span class="font-medium tabular-nums text-orange-600">— <?php echo e(number_format($invoice->global_discount_amount, 0, ',', ' ')); ?> FCFA</span>
            </div>
            <?php endif; ?>
            <div class="border-t border-gray-200 pt-3 flex justify-between">
                <span class="text-sm font-bold text-gray-900">Montant TTC</span>
                <span class="text-sm font-bold text-gray-900 tabular-nums"><?php echo e(number_format($invoice->total_ttc, 0, ',', ' ')); ?> FCFA</span>
            </div>

            
            <?php if(!empty($invoice->withholding_details)): ?>
                <?php $__currentLoopData = $invoice->withholding_details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $w): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex justify-between text-sm text-amber-700">
                    <span>Retenue <?php echo e($w['short_name'] ?? $w['name']); ?> <?php echo e(number_format($w['rate'], 2, ',', '')); ?>%</span>
                    <span class="font-medium tabular-nums">— <?php echo e(number_format($w['amount'], 0, ',', ' ')); ?> FCFA</span>
                </div>
            ...
            <div class="border-t-2 border-indigo-200 pt-3 flex justify-between">
                ...
                <span class="text-base font-bold text-indigo-700 tabular-nums"><?php echo e(number_format($invoice->net_to_pay ?: $invoice->total_ttc, 0, ',', ' ')); ?> FCFA</span>
            </div>

            <?php if($invoice->paid_amount > 0): ?>
            <div class="flex justify-between text-sm text-gray-600 border-t border-gray-100 pt-2">
                <span>Déjà payé</span>
                <span class="font-medium tabular-nums text-green-600"><?php echo e(number_format($invoice->paid_amount, 0, ',', ' ')); ?> FCFA</span>
            </div>
            <?php endif; ?>
            ...
            <div class="flex justify-between text-sm border-t border-gray-100 pt-2">
                ...
                <span class="font-bold tabular-nums text-red-700"><?php echo e(number_format($invoice->remaining_amount, 0, ',', ' ')); ?> FCFA</span>
            </div>
            <?php endif; ?>
        </div>
    </div>
            ...
            <table class="w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right text-gray-600 tabular-nums hidden md:table-cell"><?php echo e(($item->discount_percent ?? 0) > 0 ? number_format($item->discount_percent, 2, ',', ' ').'%' : '—'); ?></td>
        ...
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-900 flex items-center gap-2">
                ...
                <span class="text-xs bg-green-100 text-green-700 font-medium px-2 py-0.5 rounded-full">
                    <?php echo e($invoice->payments->count()); ?> encaissement(s)
                </span>
            </h2>
            <span class="text-sm font-semibold text-green-700 tabular-nums">
                <?php echo e(number_format($totalPaye, 0, ',', ' ')); ?> FCFA encaissés
            </span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__currentLoopData = $invoice->payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pmt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <span class="font-mono font-semibold text-green-700 text-xs"><?php echo e($pmt->number); ?></span>
                        </td>
                        <td class="px-4 py-3 text-gray-700"><?php echo e($pmt->payment_date?->format('d/m/Y') ?? '—'); ?></td>
                        <td class="px-4 py-3 text-gray-600 hidden md:table-cell"><?php echo e($pmt->paymentMethod?->name ?? '—'); ?></td>
                        <td class="px-4 py-3 text-right font-semibold tabular-nums text-green-600">
                            <?php echo e(number_format($pmt->pivot->amount ?? $pmt->amount, 0, ',', ' ')); ?> FCFA
                        </td>
                        <td class="px-4 py-3 text-gray-400 text-xs hidden lg:table-cell">
                            <?php echo e($pmt->reference ?: '—'); ?>

                        </td>
                        <td class="px-4 py-3 text-right">
                            <a href="<?php echo e(route('tresorerie.encaissements.show', $pmt)); ?>"
                ...
                <tfoot class="bg-gray-50">
                    <tr>
                        ...
                        <td class="px-4 py-2.5 text-right font-bold tabular-nums text-green-700">
                            <?php echo e(number_format($totalPaye, 0, ',', ' ')); ?> FCFA
                        </td>
                        <td colspan="2" class="px-4 py-2.5">
                            <?php if($invoice->remaining_amount > 0): ?>
                                <span class="text-xs font-medium text-red-600">
                                    Reste : <?php echo e(number_format($invoice->remaining_amount, 0, ',', ' ')); ?> FCFA
                                </span>
                            <?php else: ?>
    ...
    <?php if($invoice->creditNotes->isNotEmpty()): ?>
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-900">Avoirs liés</h2>
            <a href="<?php echo e(route('ventes.avoirs.create', ['invoice_id' => $invoice->id])); ?>"
        ...
        <table class="w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php $__currentLoopData = $invoice->creditNotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-mono font-semibold text-purple-700">
                        <a href="<?php echo e(route('ventes.avoirs.show', $cn)); ?>" class="hover:text-purple-900"><?php echo e($cn->number); ?></a>
                    </td>
                    <td class="px-4 py-3 text-gray-600"><?php echo e($cn->issued_at?->format('d/m/Y') ?? '—'); ?></td>
                    <td class="px-4 py-3 text-gray-500 text-xs hidden md:table-cell"><?php echo e($cn->reason ?? '—'); ?></td>
                    <td class="px-4 py-3 text-right tabular-nums text-purple-700 font-semibold"><?php echo e(number_format($cn->total_ttc, 0, ',', ' ')); ?> FCFA</td>
                    <td class="px-4 py-3 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium <?php echo e($cnBadges[$cn->status] ?? 'bg-gray-100 text-gray-600'); ?>">
                            <?php echo e($cnLabels[$cn->status] ?? $cn->status); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <a href="<?php echo e(route('ventes.avoirs.show', $cn)); ?>" class="p-1.5 text-gray-400 hover:text-purple-600 hover:bg-purple-50 rounded" title="Voir">
    ...
    <?php if(isset($audits) && $audits->isNotEmpty()): ?>
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-900 flex items-center gap-2">
            ...
            <span class="text-xs text-gray-400"><?php echo e($audits->count()); ?> opération(s)</span>
        </div>
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-5 py-3 flex items-start gap-3 hover:bg-gray-50/50">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold <?php echo e($cls); ?> flex-shrink-0">
                    <?php echo e($label); ?>

                </span>
                <div class="flex-1 min-w-0">
                    <div class="text-xs text-gray-600">
                        <span class="font-medium text-gray-800"><?php echo e($audit->user_name ?? 'Système'); ?></span>
                        <span class="text-gray-400">·</span>
                        <span><?php echo e($audit->created_at->format('d/m/Y H:i:s')); ?></span>
                    </div>
                    <?php if($audit->action === 'updated' && $audit->new_values): ?>
                    <?php
                        $diffFields = array_diff_key($audit->new_values, ['number' => 1]);
                    ?>
                    <?php if(!empty($diffFields)): ?>
                    <div class="mt-1 text-xs text-gray-500">
                        <?php $__currentLoopData = $diffFields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field => $newVal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                $oldVal = $audit->old_values[$field] ?? null;
                                if (is_array($newVal) || is_array($oldVal)) continue;
                            ?>
                            <div class="flex items-baseline gap-1.5">
                                <span class="font-mono font-medium text-gray-700"><?php echo e($field); ?></span>
                                <span class="text-gray-400 line-through"><?php echo e($oldVal !== null ? \Illuminate\Support\Str::limit((string) $oldVal, 50) : '∅'); ?></span>
                                <span class="text-gray-400">→</span>
                                <span class="text-gray-800"><?php echo e(\Illuminate\Support\Str::limit((string) $newVal, 50)); ?></span>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                    <?php endif; ?>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md" @click.outside="showCancelModal = false">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-base font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    Annuler la facture <?php echo e($invoice->number); ?>

                </h3>
            </div>
            <form action="<?php echo e(route('ventes.factures.cancel', $invoice)); ?>" method="POST" data-turbo="false">
                <?php echo csrf_field(); ?>
    ...
    <?php if(!in_array($invoice->status, ['brouillon','annulee','payee'])): ?>
    <?php $schedules = $invoice->paymentSchedules; ?>
    <div class="bg-white rounded-xl border border-indigo-200 overflow-hidden"
         x-data="{ tab: '<?php echo e($schedules->count() ? 'view' : 'create'); ?>', mode: 'percent', rows: [{ percent: 100, days_after: 0, label: '' }], customRows: [{ due_date: '', amount: '', label: '' }] }">
        ...
        <div class="px-4 py-3 bg-indigo-50 border-b border-indigo-200 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-medium">
                    <?php echo e($schedules->count()); ?> échéance(s)
                </span>
                <?php endif; ?>
            </div>
            <div class="flex items-center gap-2">
        ...
        <div x-show="tab === 'view'">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-indigo-50">
                    ...
                    ?>
                    <tr class="hover:bg-indigo-50 <?php echo e($isLate ? 'bg-rose-50' : ''); ?>">
                        <td class="px-4 py-2.5 text-gray-700">
                            <?php echo e($sch->label ?: ('Échéance '.$sch->installment_number)); ?>

                        </td>
                        <td class="px-4 py-2.5 text-center <?php echo e($isLate ? 'text-rose-700 font-semibold' : 'text-gray-700'); ?>">
                            <?php echo e($sch->due_date->format('d/m/Y')); ?>

                            <?php if($isLate): ?>
                            <span class="ml-1 text-xs text-rose-600 font-bold">
                                (<?php echo e(now()->diffInDays($sch->due_date)); ?>j)
                            </span>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-2.5 text-right tabular-nums text-gray-700"><?php echo e(number_format($sch->amount, 0, ',', ' ')); ?></td>
                        <td class="px-4 py-2.5 text-right tabular-nums text-green-700"><?php echo e(number_format($sch->paid_amount, 0, ',', ' ')); ?></td>
                        <td class="px-4 py-2.5 text-right tabular-nums font-bold <?php echo e($sch->remainingAmount() > 0 ? 'text-orange-600' : 'text-gray-400'); ?>">
                            <?php echo e(number_format($sch->remainingAmount(), 0, ',', ' ')); ?>

                        </td>
                        <td class="px-4 py-2.5 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-<?php echo e($sc2); ?>-100 text-<?php echo e($sc2); ?>-700">
                                <?php echo e($schLabels[$sch->status] ?? $sch->status); ?>
                ...
                <tfoot class="bg-indigo-50">
                    <tr>
                        ...
                        <td class="px-4 py-2.5 text-right tabular-nums font-bold text-indigo-800"><?php echo e(number_format($schedules->sum('amount'), 0, ',', ' ')); ?></td>
                        <td class="px-4 py-2.5 text-right tabular-nums font-bold text-green-700"><?php echo e(number_format($schedules->sum('paid_amount'), 0, ',', ' ')); ?></td>
                        <td class="px-4 py-2.5 text-right tabular-nums font-bold text-orange-600"><?php echo e(number_format($schedules->sum(fn($s) => $s->remainingAmount()), 0, ',', ' ')); ?></td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="px-4 py-3 border-t border-indigo-100 flex justify-end">
                <form action="<?php echo e(route('ventes.factures.schedules.destroy-all', $invoice)); ?>" method="POST"
        ...
        <div x-show="tab === 'create'" class="p-4 space-y-4">
            ...
            <div x-show="mode === 'percent'">
                <form action="<?php echo e(route('ventes.factures.schedules.store', $invoice)); ?>" method="POST" class="space-y-3">
                    <?php echo csrf_field(); ?>
            ...
            <div x-show="mode === 'custom'">
                <form action="<?php echo e(route('ventes.factures.schedules.store-custom', $invoice)); ?>" method="POST" class="space-y-3">
                    <?php echo csrf_field(); ?>
                    <?php $schedBasis = (int) ($invoice->net_to_pay ?: $invoice->total_ttc); ?>
                    <p class="text-xs text-gray-500">
                        Net à payer : <strong class="tabular-nums"><?php echo e(number_format($schedBasis, 0, ',', ' ')); ?> FCFA</strong>
                    ...
                    <div class="flex items-center justify-between pt-1">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs"
                                  :class="customRows.reduce((s,r)=>s+parseInt(r.amount||0),0)===<?php echo e($schedBasis); ?> ? 'text-green-600 font-medium' : 'text-orange-500'">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.document.related','data' => ['links' => $relatedLinks]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['links' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($relatedLinks)]); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.workflow.status-badge','data' => ['status' => $invoice->status,'label' => $invoice->status_label]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
        ...
        <?php if($invoice->rejection_reason): ?>
            <div class="mb-4 rounded-lg bg-orange-50 border border-orange-200 p-3 text-sm text-orange-800">
                <strong>Motif de refus :</strong> <?php echo e($invoice->rejection_reason); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.workflow.action-buttons','data' => ['document' => $invoice,'submitRoute' => 'ventes.factures.submit','validateRoute' => 'ventes.factures.validate-internal','rejectRoute' => 'ventes.factures.reject-internal','cancelRoute' => 'ventes.factures.cancel-internal','routeParam' => $invoice->id]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['document' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($invoice),'submitRoute' => 'ventes.factures.submit','validateRoute' => 'ventes.factures.validate-internal','rejectRoute' => 'ventes.factures.reject-internal','cancelRoute' => 'ventes.factures.cancel-internal','routeParam' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($invoice->id)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.workflow.history','data' => ['document' => $invoice]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['document' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($invoice)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.audit.timeline','data' => ['model' => \App\Models\Invoice::class,'id' => $invoice->id]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['model' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(\App\Models\Invoice::class),'id' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($invoice->id)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php echo $__env->make('layouts.erp', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\iboa\resources\views/ventes/factures/show.blade.php ENDPATH**/ ?>