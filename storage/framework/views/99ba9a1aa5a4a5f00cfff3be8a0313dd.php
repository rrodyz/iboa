...
<td class="px-3 py-2">
    ...
    <button type="button"
            @click="
                ...
                item._ps_open = true;
                item._ps_search = '';
                $nextTick(() => { const el = document.getElementById('ps_<?php echo e($pfx); ?>_' + item._key); if (el) el.focus(); });
            "
            class="w-full flex items-center justify-between gap-1 border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm bg-white hover:border-<?php echo e($accent); ?>-400 focus:outline-none focus:ring-1 focus:ring-<?php echo e($accent); ?>-500 min-w-[160px] transition-colors"
            :class="item._ps_open ? 'border-<?php echo e($accent); ?>-500 ring-1 ring-<?php echo e($accent); ?>-500' : ''">
    ...
    <div x-show="item._ps_open" x-cloak
        ...
        <div class="p-2.5 border-b border-gray-100 bg-gray-50/80 flex-shrink-0">
            <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-2.5 py-1.5 shadow-sm
                        focus-within:border-<?php echo e($accent); ?>-400 focus-within:ring-1 focus-within:ring-<?php echo e($accent); ?>-300 transition">
                <svg class="w-3.5 h-3.5 text-<?php echo e($accent); ?>-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <input :id="'ps_<?php echo e($pfx); ?>_' + item._key"
        ...
        <ul class="flex-1 overflow-y-auto divide-y divide-gray-50 overscroll-contain">
            ...
            <li @click="item.product_id = ''; item._ps_open = false; item._ps_search = ''; onProductChange(index)"
                ...
                :class="!item.product_id ? 'bg-<?php echo e($accent); ?>-50 text-<?php echo e($accent); ?>-600' : 'text-gray-500'">
            ...
            <template x-for="p in window.psFilter(products, item._ps_search).slice(0, 60)" :key="p.id">
                <li @click="item.product_id = String(p.id); item._ps_open = false; item._ps_search = ''; onProductChange(index)"
                    class="px-3 py-2.5 cursor-pointer hover:bg-<?php echo e($accent); ?>-50/70 transition-colors"
                    :class="String(item.product_id) === String(p.id) ? 'bg-<?php echo e($accent); ?>-50' : ''">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <svg x-show="String(item.product_id) === String(p.id)"
                             class="w-4 h-4 text-<?php echo e($accent); ?>-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="px-3 py-1.5 border-t border-gray-100 bg-gray-50/80 flex items-center justify-between flex-shrink-0"
            ...
            <button type="button" @click.stop="item._ps_search = ''"
                    class="text-xs text-<?php echo e($accent); ?>-500 hover:opacity-70">Effacer</button>
        </div>
    </div>
...
<?php /**PATH C:\laragon\www\iboa\resources\views/ventes/partials/_product_combobox.blade.php ENDPATH**/ ?>