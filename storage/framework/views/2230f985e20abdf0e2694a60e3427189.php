<?php $__env->startSection('title', 'Exercices fiscaux'); ?>
...
<div x-data="{
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table class="w-full divide-y divide-gray-100 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php $__empty_1 = true; $__currentLoopData = $years; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fy): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                    $statusConfig = [
                ...
                ?>
                <tr class="<?php echo e($fy->is_current ? 'bg-blue-50/40' : ''); ?> hover:bg-gray-50 transition-colors">
                    <td class="px-5 py-3">
                        <span class="font-semibold text-gray-900"><?php echo e($fy->label); ?></span>
                        <?php if($fy->is_current): ?>
                        <span class="ml-2 text-xs font-medium text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded">Courant</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-5 py-3 text-gray-600"><?php echo e($fy->starts_at->format('d/m/Y')); ?></td>
                    <td class="px-5 py-3 text-gray-600"><?php echo e($fy->ends_at->format('d/m/Y')); ?></td>
                    <td class="px-5 py-3 text-gray-500 text-xs"><?php echo e($days); ?> j (≈ <?php echo e($months); ?> mois)</td>
                    <td class="px-5 py-3 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium <?php echo e($statusConfig[0]); ?>">
                    ...
                    <td class="px-5 py-3">
                        ...
                        <div class="flex items-center justify-end gap-2 flex-wrap">
                            <?php if(!$fy->is_current && $fy->status !== 'archive'): ?>
                            <form method="POST" action="<?php echo e(route('settings.fiscal-years.set-current', $fy)); ?>"><?php echo csrf_field(); ?>
                                <button type="submit" class="text-xs text-blue-600 hover:text-blue-800 font-medium whitespace-nowrap">Définir courant</button>
                            </form>
                            <?php endif; ?>
                            <?php if($fy->status === 'ouvert'): ?>
                            <button @click="openEdit(<?php echo e($fy->id); ?>, '<?php echo e(addslashes($fy->label)); ?>', '<?php echo e($fy->starts_at->format('Y-m-d')); ?>', '<?php echo e($fy->ends_at->format('Y-m-d')); ?>')"
                                    class="text-xs text-gray-500 hover:text-gray-700 font-medium">Modifier</button>
                            <form method="POST" action="<?php echo e(route('settings.fiscal-years.close', $fy)); ?>"
                                  onsubmit="return confirm('Clôturer l\'exercice <?php echo e(addslashes($fy->label)); ?> ? Cette action est irréversible.')"><?php echo csrf_field(); ?>
                                <button type="submit" class="text-xs text-orange-600 hover:text-orange-800 font-medium">Clôturer</button>
                            </form>
                            <?php endif; ?>
                            <?php if($fy->status === 'cloture'): ?>
                                <?php if($ranGenerated): ?>
                                ...
                                <?php else: ?>
                                <form method="POST" action="<?php echo e(route('settings.fiscal-years.report-a-nouveau', $fy)); ?>"
                                      onsubmit="return confirm('Générer le Report à nouveau pour l\'exercice <?php echo e(addslashes($fy->label)); ?> ? Cela créera une écriture d\'ouverture dans l\'exercice suivant.')"><?php echo csrf_field(); ?>
                                ...
                                <?php endif; ?>
                            <?php endif; ?>
                            <?php if($fy->status === 'cloture' && !$fy->is_current): ?>
                            <form method="POST" action="<?php echo e(route('settings.fiscal-years.archive', $fy)); ?>"
                                  onsubmit="return confirm('Archiver définitivement cet exercice ?')"><?php echo csrf_field(); ?>
    ...
    <div x-show="modal === 'form'" x-transition
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <form method="POST"
                  :action="editId ? ('<?php echo e(url('parametres/exercices')); ?>/' + editId) : '<?php echo e(route('settings.fiscal-years.store')); ?>'"
                  class="space-y-4">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.erp', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\iboa\resources\views/settings/fiscal-years.blade.php ENDPATH**/ ?>