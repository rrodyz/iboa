...
class MarginsReportExport implements FromArray, WithTitle, WithColumnWidths, WithEvents
{
    ...
    public function title(): string { return 'Marges '.$this->from.' au '.$this->to; }
    ...
    public function columnWidths(): array
    {
        return [
            ...
            'C' => 14, // Qté vendue
            'D' => 20, // CA HT (FCFA)
            'E' => 22, // Coût achat (FCFA)
            'F' => 22, // Marge brute (FCFA)
            'G' => 16, // Taux marge (%)
        ];
    }
    ...
    private function build(): void
    {
        ...
        $totalMarge   = (int) $this->products->sum('marge_brute');
        $nbProduits   = $this->products->count();

        // ── Row 1 : document header ───────────────────────────────────────────
        $this->push(
            [$this->companyNameCell($company), null, null, 'RAPPORT DES MARGES', null, null, 'Au '.now()->format('d/m/Y')],
            self::T_HEADER
        );
        ...
        $this->push([
            $nbProduits.' produit(s)', null,
            null,
            'CA HT total : '.$totalCaHt,
            'Coût total : '.$totalCout,
            'Marge totale : '.$totalMarge,
            null,
        ], self::T_TOTALS);
        ...
        $this->push([
            'Référence', 'Produit', 'Qté vendue', 'CA HT (FCFA)', 'Coût achat (FCFA)', 'Marge brute (FCFA)', 'Taux marge (%)',
        ], self::T_COLHDR);
    ...
    private function applyStyles(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws): void
    {
        ...
        $ws->getHeaderFooter()->setOddHeader('&L&BRapport des Marges&R&P / &N');
