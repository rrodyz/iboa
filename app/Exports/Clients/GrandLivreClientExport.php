...
class GrandLivreClientExport implements FromArray, WithTitle, WithColumnWidths, WithEvents
{
    ...
    public function title(): string { return 'Grand livre clients'; }
    public function array(): array  { return $this->rows; }

    public function columnWidths(): array
    {
        return ['A' => 13, 'B' => 18, 'C' => 8, 'D' => 18, 'E' => 44, 'F' => 18, 'G' => 18, 'H' => 18];
    }
    ...
    private function build(): void
    {
        ...
        $periodText = 'Écritures validées — comptes clients';
        ...
        foreach ($accounts as $account) {
            ...
            foreach ($account['lines'] as $item) {
                ...
                $this->push([
                    ...
                    $l->label ?: ($l->journalEntry?->description ?? ''),
                    (int)$l->debit  > 0 ? (int)$l->debit  : null,
                    (int)$l->credit > 0 ? (int)$l->credit : null,
                    $item['solde'],
                ], self::T_DATA);
            ...
            // Account subtotal
            $this->push([
                'Total ' . $account['code'], null, null, null, null,
                $account['total_d'] ?: null,
                $account['total_c'] ?: null,
                $account['solde_fin'],
            ], self::T_ACC_TOTAL);
    ...
    private function loadAccounts(): array
    {
        ...
        $lines   = $query->orderBy(JournalEntry::select('entry_date')->whereColumn('id', 'journal_entry_lines.journal_entry_id'), 'asc')->orderBy('journal_entry_id')->get();
        ...
        foreach ($grouped as $code => $accountLines) {
            ...
            if ($this->dateFrom) {
                $openLines = JournalEntryLine::whereHas('account', fn($q) => $q->where('code', $code))
                    ->whereHas('journalEntry', fn($q) => $q->where('status', 'valide')->whereDate('entry_date', '<', $this->dateFrom))->get();
    ...
    private function applyStyles(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws): void
    {
        ...
        // Number format on account totals
        ...
        $ws->getHeaderFooter()->setOddHeader('&L&BGrand Livre Clients&R&P / &N');
    ...
    {
        $ws->mergeCells("A{$r}:C{$r}"); $ws->mergeCells("D{$r}:G{$r}");
        $ws->getStyle("A{$r}:H{$r}")->applyFromArray(['font' => ['bold' => true, 'size' => 10, 'color' => ['rgb' => 'FFFFFF']], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => '312E81']], 'alignment' => ['vertical' => Alignment::VERTICAL_CENTER]]);
        $ws->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
    ...
    {
        $ws->getStyle("A{$r}:H{$r}")->applyFromArray(['font' => ['bold' => true, 'size' => 8, 'color' => ['rgb' => '3730A3']], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'EEF2FF']], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER], 'borders' => ['bottom' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => '818CF8']]]]);
        $ws->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
    ...
    private function sAccTotal($ws, int $r): void
    {
        ...
        $ws->getStyle("A{$r}:H{$r}")->applyFromArray(['font' => ['bold' => true, 'size' => 9, 'color' => ['rgb' => '312E81']], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'EEF2FF']], 'borders' => ['top' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => '818CF8']]], 'alignment' => ['vertical' => Alignment::VERTICAL_CENTER]]);
        $ws->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
        foreach (['F', 'G', 'H'] as $c) { $ws->getStyle($c . $r)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT); }
        $ws->getRowDimension($r)->setRowHeight(16);
    }
