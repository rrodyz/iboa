...
class ClientsExport implements FromArray, WithTitle, WithColumnWidths, WithEvents
{
    ...
    private const COLS     = 18; // A–R
    private const LAST_COL = 'R';
    private const COLOR    = '2563EB';
    ...
    private function build(): void
    {
        ...
        $totalSolde = (int) $clients->sum('balance');

        // ── Row 1 : document header ───────────────────────────────────────────
        $this->push(
            [$this->companyNameCell($company), null, null, null, null, null, null, null, 'RÉPERTOIRE CLIENTS', null, null, null, null, null, null, null, null, 'Au '.now()->format('d/m/Y')],
            self::T_HEADER
        );

        // ── Row 2 : period + filters ──────────────────────────────────────────
        $parts = ['Export du répertoire clients'];
        if (!empty($this->filters['search'])) {
            $parts[] = 'Recherche : "'.$this->filters['search'].'"';
        }
        if (!empty($this->filters['type'])) {
            $parts[] = 'Type : '.ucfirst($this->filters['type']);
        }
        ...
        $this->push(
            [$this->companyLegalLine($company, $period), null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            self::T_PERIOD
        );

        // ── Row 3 : summary totals ────────────────────────────────────────────
        $this->push([
            $clients->count().' client(s)', null, null, null, null, null, null, null,
        ...
        $this->push([
            'Code', 'Type', 'Nom', 'Nom commercial', 'Email', 'Téléphone', 'Mobile',
            'Adresse', 'Ville', 'Pays', 'IFU', 'RCCM',
            'Limite crédit (FCFA)', 'Délai paiement (j)', 'Remise défaut (%)',
            'Solde dû (FCFA)', 'Statut', 'Créé le',
        ], self::T_COLHDR);
        ...
        foreach ($clients as $client) {
            $this->push([
                ...
                $client->ifu ?? '—',
                $client->rccm ?? '—',
                (int) $client->credit_limit,
                (int) $client->payment_days,
                (float) $client->default_discount,
                (int) $client->balance,
    ...
    private function applyStyles(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws): void
    {
        ...
        $ws->getHeaderFooter()->setOddHeader('&L&BRépertoire Clients&R&P / &N');
    ...
    private function styleHeader(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws, int $r, string $lastCol): void
    {
        ...
        $ws->mergeCells("E{$r}:P{$r}");
        $ws->mergeCells("Q{$r}:{$lastCol}{$r}");
        ...
        $ws->getStyle("Q{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
    ...
    private function styleTotals(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws, int $r, string $lastCol, string $numFmt): void
    {
        ...
        $ws->getStyle("P{$r}")->applyFromArray([
            'font'      => ['bold' => true, 'size' => 10, 'color' => ['rgb' => 'DBEAFE']],
            'alignment' => ['horizontal' => Alignment::HORIZONTAL_RIGHT],
        ]);
        $ws->getStyle("P{$r}")->getNumberFormat()->setFormatCode($numFmt);
    ...
    private function styleColHeader(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws, int $r, string $lastCol): void
    {
        ...
        foreach (['A', 'B', 'C', 'D', 'E', 'H', 'I', 'J', 'K', 'L'] as $c) {
    ...
    private function styleData(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $ws, int $r): void
    {
        ...
        foreach (['M', 'N', 'O', 'P'] as $c) {
            $ws->getStyle("{$c}{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        }
        $ws->getStyle('Q'.$r)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
