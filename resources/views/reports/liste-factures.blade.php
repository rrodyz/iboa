...
<div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Liste des factures</h1>
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div class="mt-3 flex gap-2">
            ...
            <a href="{{ route('reports.liste-factures') }}" class="border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm px-3 py-2 rounded-lg">Réinitialiser</a>
        </div>
    </form>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="mt-1 text-xl font-bold text-indigo-700">{{ $rows->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <table class="w-full text-sm">
            <thead class="bg-indigo-600 text-white">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right font-semibold">Restant</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2.5 text-center text-gray-600">{{ $r->due_at?->format('d/m/Y') ?? '—' }}</td>
                    ...
                    <td class="px-4 py-2.5 text-right tabular-nums font-medium text-gray-900">{{ number_format($r->total_ttc, 0, ',', ' ') }}</td>
                    <td class="px-4 py-2.5 text-right tabular-nums text-emerald-700">{{ number_format($r->paid_amount, 0, ',', ' ') }}</td>
                    <td class="px-4 py-2.5 text-right tabular-nums font-semibold {{ $r->remaining_amount > 0 ? 'text-rose-700' : 'text-gray-400' }}">
            ...
            <tfoot class="bg-indigo-900 text-white font-bold">
                <tr>
                    <td class="px-4 py-3" colspan="5">TOTAL ({{ $rows->count() }} facture{{ $rows->count() > 1 ? 's' : '' }})</td>
