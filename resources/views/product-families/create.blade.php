...
<div class="max-w-2xl">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Nouvelle famille de produits</h1>
        <p class="text-sm text-gray-500 mt-1">Organisez vos articles en familles et sous-familles</p>
    </div>
    ...
    <form method="POST" action="{{ route('product-families.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        @csrf
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Laissez vide pour créer une famille de niveau 1</p>
        </div>
        ...
        <div>
            ...
            <input type="text" id="code" name="code" value="{{ old('code') }}"
                   ...
                   placeholder="Ex: ELEC, INFO… (optionnel)">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3"
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent resize-none"
                      placeholder="Description optionnelle…">{{ old('description') }}</textarea>
        </div>

        {{-- ═══════════════════════════════════════════════════════
             Comptes comptables (SYSCOA / OHADA)
             ─ Les écritures comptables générées par les ventes
               et les achats des articles de cette famille utiliseront
                ...
                <div>
                    ...
                    <select id="sale_account_id" name="sale_account_id"
                        ...
                        @foreach($accounts['sale'] as $acc)
                            <option value="{{ $acc->id }}" {{ old('sale_account_id') == $acc->id ? 'selected' : '' }}>
                                {{ $acc->code }} — {{ $acc->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="purchase_account_id" name="purchase_account_id"
                        ...
                        @foreach($accounts['purchase'] as $acc)
                            <option value="{{ $acc->id }}" {{ old('purchase_account_id') == $acc->id ? 'selected' : '' }}>
                                {{ $acc->code }} — {{ $acc->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="stock_account_id" name="stock_account_id"
                        ...
                        @foreach($accounts['stock'] as $acc)
                            <option value="{{ $acc->id }}" {{ old('stock_account_id') == $acc->id ? 'selected' : '' }}>
                                {{ $acc->code }} — {{ $acc->name }}
                            </option>
                        @endforeach
                    </select>
